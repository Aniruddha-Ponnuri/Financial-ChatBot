...
def generate_candidates(question, history, knowledge_base_prompt, n_candidates, stock_context=""):
    ...
    try:
        ...
        logger.info(f"Stock context: {'present (' + str(len(stock_context)) + ' chars)' if stock_context else 'not present'}")
...
def select_best_candidate(question, candidates):
    ...
    try:
        ...
        if not scores or len(scores) != len(candidates):
            logger.error(f"Invalid scores returned: expected {len(candidates)}, got {len(scores) if scores else 0}")
        ...
        logger.info(f"Selected candidate {best_idx+1}/{len(candidates)} with score {best_score:.4f}")
        logger.info(f"All candidate scores: {[f'{s:.4f}' for s in scores]}")
        logger.info(f"Score range: min={min(scores):.4f}, max={max(scores):.4f}, mean={sum(scores)/len(scores):.4f}")
...
def ask_question():
    ...
    try:
        ...
        logger.info(f"Question received: '{question[:100] if question else 'None'}...' (length: {len(question) if question else 0})")
        ...
        try:
            ...
            logger.info(f"Stock extraction result - symbols: {symbols}, is_stock_query: {is_stock_query}")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"History summarized successfully (length: {len(summarized_history)} chars)")
        except Exception as e:
            ...
            updated_history = f"{history}\nHuman: {question}\nAI: {formatted_answer}"
            summarized_history = updated_history[-1000:] if len(updated_history) > 1000 else updated_history
...
def summarize_conversation(conversation, max_tokens=1000):
    ...
    try:
        ...
        if not summarization_prompt:
            ...
            summarization_prompt = "You are a helpful assistant that summarizes conversations concisely."
        ...
        logger.info(f"Compression ratio: {len(summary)}/{len(cleaned_conversation)} = {(len(summary)/len(cleaned_conversation)*100):.1f}%")
        logger.info("=" * 60)
        return summary
