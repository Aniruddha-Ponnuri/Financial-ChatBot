...
class RewardModel:
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            if os.path.exists(self.clf_path):
                ...
                self.logger.info(f"Classifier classes: {self.clf.classes_ if hasattr(self.clf, 'classes_') else 'Not yet trained'}")
            else:
    ...
    def predict_scores(self, texts):
        ...
        if self.clf is None:
            self.logger.warning("No trained reward model available - returning uniform scores (0.5)")
        ...
        try:
            ...
            if 1 in self.clf.classes_:
                ...
                self.logger.warning("Classifier has not seen both classes yet - returning uniform scores")
    ...
    def update(self, texts, labels):
        ...
        if len(texts) != len(labels):
            self.logger.error(f"Mismatch between texts ({len(texts)}) and labels ({len(labels)}) length")
            return
        ...
        try:
            ...
            self.logger.info(f"Label distribution - Positive: {positive_count}, Negative: {negative_count}")
    ...
    def _save_model(self):
        ...
        try:
            ...
            self.logger.info(f"Reward model saved successfully (total training samples: {self.training_count})")
        except Exception as e:
