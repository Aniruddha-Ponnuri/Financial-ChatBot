...
class Config_Reader:
    ...
    def _get_config_value(self, key_name: str) -> str:
        ...
        if self.logger:
            self.logger.info(
                f"Config value retrieved for {key_name}: {'***' if 'KEY' in key_name or 'PASSWORD' in key_name else value}"
            )
        return value
...
class BotConfig:
    ...
    def get(self, key_path: str, default=None):
        ...
        self.logger.info(
            f"Retrieved config value for '{key_path}': {value if not isinstance(value, (dict, list)) else type(value).__name__}"
        )
        return value
